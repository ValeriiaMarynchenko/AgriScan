...
# --- 1. КОНФІГУРАЦІЯ ПРОЕКТУ КУКУРУДЗИ ---
# Класи хвороб кукурудзи, визначені за назвами ваших папок
TRAIN_DIRS = [
    "Corn___Common_Rust",
...
CLASSES = TRAIN_DIRS
NUM_CLASSES = len(CLASSES)  # Загальна кількість класів: 4
...
# КОРЕНЕВА ДИРЕКТОРІЯ ДЛЯ КЛАСІВ КУКУРУДЗИ
DATA_ROOT = "D:\\datasets\\corn_potato_wheat\\Corn"

# Гіперпараметри навчання
LEARNING_RATE = 1e-4
BATCH_SIZE = 4
NUM_EPOCHS = 5  # Збільшимо епохи, оскільки класів менше
...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear(512, num_classes)  # num_classes = 4 для кукурудзи
        )
...
class CornDiseaseDataset(Dataset):
    """Клас для завантаження RGB зображень кукурудзи для класифікації."""
    ...
    def __init__(self, rgb_dir, class_dirs, transform=get_transforms):
        ...
        print(f"Завантажено {len(self.images)} зображень для класифікації кукурудзи.")
...
def save_checkpoint(state, filename="cnn_corn_classification_best.pth.tar"):
...
def train_fn(loader, model, optimizer, loss_fn, scaler, device):
    ...
    loop = tqdm(loader, desc="Training Corn")
    total_loss = 0
...
def main():
    ...
    print(f"Завдання: Класифікація Кукурудзи ({NUM_CLASSES} класів)")
    ...
    best_loss = float('inf')
    CHECKPOINT_PATH = "cnn_corn_classification_best.pth.tar"
